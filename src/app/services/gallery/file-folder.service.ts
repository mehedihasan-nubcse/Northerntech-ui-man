...
export class FileFolderService {
  ...
  getAllFileFolders(filterData: FilterData, searchQuery?: string) {
    ...
    return this.httpClient.post<{ data: FileFolder[], count: number, success: boolean }>(API_BRAND + 'all-file-folders', filterData, {params});
  }
  ...
  updateFileFolderById(id: string, data: FileFolder) {
    return this.httpClient.put<{ message: string, success: boolean }>(API_BRAND + 'update-file-folder/' + id, data);
  }

  updateMultipleFileFolderById(ids: string[], data: FileFolder) {
    const mData = {...{ids: ids}, ...data}
    return this.httpClient.put<ResponsePayload>(API_BRAND + 'update-multiple-file-folder-by-id', mData);
  }

  deleteFileFolderById(id: string) {
    return this.httpClient.delete<ResponsePayload>(API_BRAND + 'delete-file-folder/' + id);
  }

  deleteMultipleFileFolderById(ids: string[]) {
    return this.httpClient.post<ResponsePayload>(API_BRAND + 'delete-multiple-file-folder-by-id', {ids: ids});
  }
