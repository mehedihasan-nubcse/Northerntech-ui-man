...
export class GalleryService {
  ...
  getAllGalleries(filterData: FilterData, searchQuery?: string) {
    ...
    return this.httpClient.post<{ data: Gallery[], count: number, success: boolean }>(API_BRAND + 'get-all-by-shop', filterData, {params});
  }
  ...
  updateGalleryById(id: string, data: Gallery) {
    return this.httpClient.put<{ message: string, success: boolean }>(API_BRAND + 'update-gallery/' + id, data);
  }

  updateMultipleGalleryById(ids: string[], data: Gallery) {
    const mData = {...{ids: ids}, ...data}
    return this.httpClient.put<ResponsePayload>(API_BRAND + 'update-multiple-gallery-by-id', mData);
  }

  deleteGalleryById(id: string) {
    return this.httpClient.delete<ResponsePayload>(API_BRAND + 'delete-gallery/' + id);
  }

  deleteMultipleGalleryById(ids: string[]) {
    return this.httpClient.post<ResponsePayload>(API_BRAND + 'delete-multiple-gallery-by-id', {ids: ids});
  }
