...
export class BrandService {
  ...
  addBrandByShop(data: Brand):Observable<ResponsePayload> {
  ...
  getAllBrands(filterData: FilterData, searchQuery?: string) {
    ...
    return this.httpClient.post<{ data: Brand[], count: number, success: boolean }>(API_BRAND + 'get-all-by-shop/', filterData, {params});
  }
  getAllBrands1(filterData: FilterData, searchQuery?: string) {
    let params = new HttpParams();
    if (searchQuery) {
      params = params.append('q', searchQuery);
    }
