...
export class SalesReturnService {
  ...
  getAllSalesReturn(filterData: FilterData, searchQuery?: string) {
    let params = new HttpParams();
    if (searchQuery) {
      params = params.append('q', searchQuery);
    }
    return this.httpClient.post<{ data: NewSalesReturn[], calculation: any, success: boolean }>(API_NEW_SALES_RETURN + 'get-all-by-shop/', filterData, {params});
  }

  getSalesReturnById(id: string, select?: string) {
    let params = new HttpParams();
    if (select) {
  ...
  deleteMultipleSalesReturnById(ids: string[], checkUsage?: boolean) {
    let params = new HttpParams();
    if (checkUsage) {
      params = params.append('checkUsage', checkUsage);
    }
    return this.httpClient.post<ResponsePayload>(API_NEW_SALES_RETURN + 'delete-multiple', { ids: ids }, { params });
  }
