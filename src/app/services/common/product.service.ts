...
export class ProductService {
  ...
  getAllProducts(filterData: FilterData, searchQuery?: string) {
    ...
    return this.httpClient.post<{ data: Product[], count: number, success: boolean, calculation: ProductCalculation,totalQuantity:any }>(API_PRODUCT + 'get-all-by-shop/', filterData, { params });
  }

  getAllGroupProducts(filterData: FilterData, searchQuery?: string) {
    let params = new HttpParams();
    if (searchQuery) {
      params = params.append('q', searchQuery);
    }
    return this.httpClient.post<{ data: Product[], count: number, success: boolean, calculation: ProductCalculation,totalQuantity:any }>(API_PRODUCT + 'get-all-by-shop-group-product/', filterData, { params });
  }
  ...
  getSimilarProducts(productId: string): Observable<any> {
    return this.httpClient.get<any>(API_PRODUCT + 'similar/' + productId);
  }
