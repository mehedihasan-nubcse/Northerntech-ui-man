...
import {CURRENCY_TYPES, defaultUploadImage, DISCOUNT_TYPES} from '../../../core/utils/app-data';
...
export class ShopInformationComponent implements OnInit, OnDestroy {
  ...
  private setData() {
    ...
    if (this.shopInformation.metaLogo) {
      this.pickedImage1 = this.shopInformation.metaLogo;
    }
    this.dataForm.patchValue(this.shopInformation);
  }
  ...
  public openGalleryDialog1() {
    ...
    dialogRef.afterClosed().subscribe(dialogResult => {
      if (dialogResult) {
        if (dialogResult.data && dialogResult.data.length > 0) {
          const image: Gallery = dialogResult.data[0] as Gallery;
          this.dataForm.patchValue({metaLogo: image.url});
