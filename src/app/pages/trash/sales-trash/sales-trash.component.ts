...
import {NoteComponent} from "../../sales/sale/note/note.component";
import {SalesLogService} from "../../../services/common/sales-log.service";

@Component({
  selector: 'app-sales-trash',
  templateUrl: './sales-trash.component.html',
  styleUrls: ['./sales-trash.component.scss'],
  providers:[DatePipe]
})
export class SalesTrashComponent implements OnInit {
  // Admin Base Data
  adminId: string;
  role: string;
  ...
  constructor(
    ...
    private saleService: SalesLogService,
    ...
    this.subForm = formValue
      .pipe(
        ...
        switchMap((data) => {
          ...
          return this.saleService.getAllSalesLogs(
            filterData,
            this.searchQuery
          );
        })
  ...
  deleteSaleById(id: any) {
    this.subDataTwo = this.saleService.deleteSalesLogById(id).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  private deleteMultipleSaleById() {
    ...
    this.subDataThree = this.saleService
      .deleteMultipleSalesLogById(this.selectedIds)
      .subscribe({
        next: (res) => {
          this.spinner.hide();
          if (res.success) {
  ...
  private restoreMultipleProductById() {
    this.spinner.show();
    this.subDataTwo = this.saleService
  ...
  public updateSalesById(id?: string, data?: any, event?: MouseEvent) {
    event.stopImmediatePropagation();
    this.subGetData1 = this.saleService.updateSalesLogById(id, data).subscribe({
      next: res => {
        if (res.success) {
