...
import {OutStockLogService} from "../../../services/common/out-stock-log.service";
import {debounceTime, distinctUntilChanged, pluck, switchMap} from "rxjs/operators";
import {Pagination} from "../../../interfaces/core/pagination";

@Component({
  selector: 'app-out-stock-trash',
  templateUrl: './out-stock-trash.component.html',
  styleUrls: ['./out-stock-trash.component.scss'],
  providers:[DatePipe]
})
export class OutStockTrashComponent implements OnInit, OnDestroy {
  // Admin Base Data
  adminId: string;
  role: string;
  ...
  damages: { _id: string, data: any[], total: number }[] = [];
  holdPrevData: any[] = [];
  id?: string;
  ...
  constructor(
    ...
    private damageService: OutStockLogService,
    ...
    this.subForm = formValue
      ...
      .subscribe({
        next: (res) => {
          ...
          // this.allPurchaseByDate = this.damages.a;
  ...
  private getAllDamage() {
    ...
    this.subDataOne = this.damageService
      .getAllOutStockLogs(filter, null)
      .subscribe({
        next: (res) => {
  ...
  private deleteMultipleDamageById() {
    ...
    this.subDataThree = this.damageService
      .deleteMultipleOutStockLogById(this.selectedIds)
      .subscribe({
        next: (res) => {
          this.spinner.hide();
          if (res.success) {
  ...
  private restoreMultipleProductById() {
    this.spinner.show();
    this.subDataTwo = this.damageService
