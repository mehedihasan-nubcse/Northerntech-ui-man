...
export class NewSalesComponent implements OnInit, OnDestroy {
  ...
  onTabClick(value: string) {
    if (this.selectedTabs.includes(value)) {
      ...
      // this.multiPayment.push({name: value, amount: 0})
      if (value === 'visa-card') {
        this.visaCharge = (this.grandTotal * 0.03);
      }
      this.multiPayment.push({name: value, amount: this.grandTotal})
  ...
  updateAmount(index: number) {
    // Push the grandTotal to data.amount for the specific index
    // console.log("updateAmount")
    this.multiPayment[index].amount = 0;
  }
  ...
  onSubmit() {
    ...
    if (this.multiPayment.length) {
      for (const data of this.multiPayment) {
        if (data?.amount === null) {
          this.uiService.warn('Please input payment amount');
          return;
        }
      }
    ...
    if (!this.multiPayment.length) {
      this.uiService.warn('Please select payment method');
      return;
    }
    ...
    this.saleData = {
      ...
      visaCharge: this.visaCharge ?? 0,
      total: this.grandTotal,
      subTotal: this.subTotal,
      paidAmount: this.grandTotal,
      totalPurchasePrice: this.purchaseTotalAmount,
      month: this.utilsService.getDateMonth(false, new Date(this.soldDate)),
      year: new Date(this.soldDate).getFullYear(),
      status: 'Sale',
  ...
  private addSale() {
    this.isLoading = true;
    if (this.multiPayment.length === 1 && this.multiPayment[0].name === 'cash') {
      this.multiPayment[0].amount = this.grandTotal;
    }
    this.subDataFive = this.newSalesService.addSale({...this.saleData, ...{multiPayment: this.multiPayment}}).subscribe({
      next: (res) => {
      ...
      .subscribe({
        next: (res => {
          ...
          if (res.data) {
            ...
            this.visaCharge = this.sale.visaCharge;
            this.setFormValue();
          }
        }),
        error: (error => {
  ...
  private updateSaleById() {
    ...
    this.subDataFour = this.newSalesService.updateSaleById(this.sale._id, {...this.saleData, ...{multiPayment: this.multiPayment}})
      .subscribe({
        next: (res => {
          this.isLoading = false;
          if (res.success) {
  ...
  incrementQuantity(index: number, data: Product) {
    ...
    if (data.quantity > this.products[index].soldQuantity) {
      ...
      this.updateMultiPaymentAmounts();
    } else {
  ...
  updateMultiPaymentAmounts(): void {
    const remainingTotal = this.grandTotal; // Calculate the new grand total
    const numberOfPayments = this.multiPayment.length;

    if (numberOfPayments > 0) {
      // Evenly distribute the grand total across payment methods
      const perPaymentAmount = +(remainingTotal / numberOfPayments).toFixed(2);
      this.multiPayment.forEach((payment) => {
        payment.amount = perPaymentAmount;
      });

      // Adjust for rounding issues, if any
      const totalAssigned = this.multiPayment.reduce((sum, payment) => sum + payment.amount, 0);
      if (totalAssigned !== remainingTotal) {
        const diff = remainingTotal - totalAssigned;
        this.multiPayment[0].amount += diff; // Adjust the first payment to balance the difference
      }
    }
  }
  ...
  get discount() {
    ...
    const amount = Number(
      this.products.filter(f => f.saleType === 'Sale').map(t => {
        return ((t.discountAmount ?? 0) * t.soldQuantity);
      }).reduce((acc, value) => acc + value, 0).toFixed(2)
    );
  ...
  // }
  get grandTotal() {
    const total = (this.subTotal + this.vat) - this.discount;
    return Number((total + this.visaCharge).toFixed(2));
  }
  // get grandTotal() {
  //
  ...
  get checkPaymentTypeAndGrandTotal() {
   return Number(((this.subTotal + this.vat) - this.discount).toFixed(2))  <= this.multiPayment.map(m => m.amount).reduce((acc, val) => acc + val, 0);
  }
  ...
  getData(){
    if (this.multiPayment.length) {
      setTimeout(() => {
        ...
        // Update the `amount` property of the 0th index
        this.multiPayment.forEach((payment, index) => {
          if (index === 0) {
            payment.amount = this.grandTotal;
          }
        });

        // console.log("Updated multiPayment:", this.multiPayment);
      }, 500);
    }
    else {
      this.selectedTabs.push('cash')
      setTimeout(()=>{
        console.log("test")
        this.multiPayment.push({name: 'cash', amount: this.grandTotal})
      },500)
