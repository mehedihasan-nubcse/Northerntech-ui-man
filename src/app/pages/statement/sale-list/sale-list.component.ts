...
export class SaleListComponent implements OnInit, OnDestroy {
          ...
          if (res.success) {
            ...
            this.allSales.forEach(sale => {
              sale.products.forEach(product => {
                const fIndex = reports.findIndex(f => f.productId === product._id && f.soldDateString === sale.soldDateString);
                if (fIndex === -1) {
                  const data = {
                  ...
                  reports[fIndex].discount += sale.discount && sale.discount > 0 ? Number((sale.discount / sale.products.length).toFixed(2)) : 0;
                }
              })
            })
  ...
  getAvDiscount(data: any) {
    return data.map(t => t['discount']?? 0).reduce((acc, value) => acc + value, 0);
  ...
  exportToAllExcel() {
    ...
    // this.sales.forEach((f, i) => {
    //   const data = document.getElementById('statement-sale-list'+ i);
    //   const ws: XLSX.WorkSheet = XLSX.utils.table_to_sheet(data);
    //   XLSX.utils.book_append_sheet(wb, ws, f._id);
    // })
