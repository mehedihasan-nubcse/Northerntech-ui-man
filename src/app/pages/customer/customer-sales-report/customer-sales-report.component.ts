...
export class CustomerSalesReportComponent implements OnInit {
  ...
  holdPrevData: Sale[] = [];
  newSalesCount = 0;
  salesCalculation: SaleCalculation;
  holdTotalSalesCalculation: SaleCalculation;
  ...
  ngOnInit(): void {
    ...
    this.activatedRoute.paramMap.subscribe((param) => {
      ...
      if (this.id) {
        this.filter = { ...this.filter, ...{ 'customer._id': this.id } };
      }
    });
    ...
    this.subForm = formValue
      .pipe(
        ...
        switchMap((data) => {
          ...
          return this.newSalesService.getAllSale(
            filterData,
            this.searchQuery
          );
        })
  ...
  private deleteSaleById(id: any) {
    this.subDataTwo = this.newSalesService.deleteSaleById(id).subscribe({
      next: (res) => {
        if (res.success) {
  ...
  //
  //
  // //   this.subDataOne = this.newSalesService.getAllSale(filterData, this.searchQuery)
  // //     .subscribe({
  // //       next: (res => {
  ...
  //
  //   this.subDataOne = this.newSalesService
  //     .getAllSale(filterData, this.searchQuery)
  //     .subscribe({
  //       next: (res) => {
  //         const subscriptionReports = res.data;
  //
    ...
    const documentObject = {
      content: [
        ...
        },
        {
          width: '*',
          text: [
            `30 Roberts Lane # 01 -00 Singapore 218309, Phone: 09611677835/01894885631, mosimosi.com `
          ],
          style: 'p',
          margin: [0, 60, 0, 0],
          alignment: 'center',
  ...
  async getProfilePicObjectPdf() {
    return {
      image: await this.getBase64ImageFromURL('https://ftp.natco.polsonerp.com/uploads/logo/polson-pos.png'),
      width: 200,
      alignment: 'left'
    };
  }
