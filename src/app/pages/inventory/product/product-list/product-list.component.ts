...
export class ProductListComponent implements OnInit, AfterViewInit, OnDestroy {
  ...
  products?: Product[] = [];
  products2?: Product[] = [];
  productCalculation2?: any;
  productCalculation3?: any;
  products3?: Product[] = [];
    ...
    this.subForm = formValue
      .pipe(
        ...
        switchMap((data) => {
          ...
          return this.productService.getAllGroupProducts(
            filterData,
            this.searchQuery
          );
        })
  ...
  onCheckChange(event: any, index: number, id: string) {
    this.products[index].select = event; // Toggle the select property for the row
    if (event) {
  ...
  onAllSelectChange(event: MatCheckboxChange) {
    const currentPageIds = this.products.map((m) => m.firstProductId);
    if (event.checked) {
      ...
      currentPageIds.forEach((m) => {
        this.products.find((f) => f.firstProductId === m).select = false;
  ...
  private getAllProducts() {
    ...
    this.subDataOne = this.productService
      .getAllGroupProducts(filter, this.searchQuery)
      .subscribe({
        next: (res) => {
  ...
  private getAllProducts2() {
    ...
    this.subDataOne = this.productService
      .getAllGroupProducts(filterData, null)
      .subscribe({
        next: (res) => {
          if (res.success) {
            this.products2 = res.data.filter(t => t.quantity !== 0);
            // this.products2 = res.data;
            this.productCalculation2 = res.calculation;
      ...
      .subscribe({
        next: (res) => {
          if (res.success) {
            ...
            this.filterProducts3 = res.data.filter(t => t.quantity !== 0);
            // this.productCalculation = res.calculation;
            this.productCalculation3 = res.calculation;
            setTimeout(() => {
              this.isReport = false;
              this.isStockReport = false;
            }, 3000)
  ...
  exportToAllExcel() {
    ...
    this.subDataOne = this.productService.getAllGroupProducts(filterData, this.searchQuery)
      .subscribe({
        next: (res => {
          ...
          if (res.success) {
            ...
            const finalData = products.map(data => {
              return {
                ...
                'Imei': data?.imei ?? '-',
                'Qty': data?.quantity,
                'Cost Price': this.decimalPipe.transform(data?.purchasePrice, '1.0-1'), // Adjusted to show 1 decimal
                'Total Cost Price': this.decimalPipe.transform(data?.purchasePrice * (data?.quantity), '1.0-1'),
                'Sale Price': this.decimalPipe.transform(data?.salePrice, '1.0-1'),
                'Unit Profit': this.decimalPipe.transform((data?.salePrice - data?.purchasePrice), '1.0-1'),
                'Total Profit': this.decimalPipe.transform((data?.salePrice * data?.quantity) - (data?.purchasePrice * data?.quantity), '1.0-1'),    }
            })
