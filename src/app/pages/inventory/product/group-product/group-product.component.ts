...
export class GroupProductComponent implements OnInit, OnDestroy {
    ...
    if (this.searchForm) {
      this.subForm = formValue$
        .pipe(
          ...
          pluck('searchTerm'),
          // Wait for 200ms of inactivity to reduce redundant operations
          debounceTime(200),
        ...
        .subscribe((searchTerm) => {
          ...
          this.products = this.holdPrevData.filter((product) => {
            const nameMatch = product.name?.toLowerCase().includes(this.searchQuery);
            const imeiMatch = product.imei?.toLowerCase().includes(this.searchQuery);
  ...
  private getAllProducts2() {
    ...
    //     next: (res) => {
    if (this.products) {
      this.products2 = this.products.filter(t => t.quantity !== 0);
  ...
  private getAllProducts3() {
    ...
    //     next: (res) => {
    if (this.products) {
      this.products3 = this.products;
      this.filterProducts3 = this.products.filter(t => t.quantity !== 0);
      // this.productCalculation = res.calculation;
    ...
    this.subDataTwo = this.productService
      ...
      .subscribe({
        next: (res) => {
          this.spinner.hide();
          if (res.success) {
            if(this.products.length === this.selectedIds.length){
  ...
  exportStockToAllExcel() {
    ...
    if (this.products) {
      ...
      const products = this.products.filter(t => t.quantity !== 0);
